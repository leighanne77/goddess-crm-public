...
def _format_contact(
    ...
    *,
    ...
    Lynda needs to know the owner BEFORE offering a "Talk to <Owner>"
    ...
    is deliberately NOT exposed — owner_name is what Lynda speaks; the
    ...
    return {
        ...
        # expanded card view + Lynda's chat readback can use them.
...
def _redacted_view(c: Contact, *, owner_name: str | None) -> dict[str, Any]:
    ...
    are populated. The owner's name + initials are included so Lynda
    can say 'ask <Owner>'.
    """
...
def _handle_create_google_task(
    ...
    Ownership rules:
      ...
      - owner name is read from the owner User row — Lynda's prompt
